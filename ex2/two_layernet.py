...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #dZ2 = dA2
        dZ2 = np.multiply(dA2, np.int64(z2 > 0)) # relu has conditional derivatives
        #dW1 = np.matmul(a1.T,dA2) / N + 2 * reg * W1
        dW1 = np.matmul(a1.T,dZ2) / N + 2 * reg * W1
        #db1 = np.sum(dA2.T, axis=1) / N
        db1 = np.sum(np.multiply(dA2, np.int64(z2 > 0)).T, axis=1) / N # relu has conditional derivatives
