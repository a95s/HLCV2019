...
plt.bar((bin_gray1[0:-1] + bin_gray1[1:])/2, hist_gray1,width=4)
#plt.show()
plt.subplot(1,3,3)
hist_gray2, bin_gray2 = histogram_module.normalized_hist(img_gray, num_bins_gray)

plt.bar((bin_gray2[0:-1] + bin_gray2[1:])/2, hist_gray2,width=4)
plt.show()
...
#print(hist_rgb1)
plt.bar(np.array(range(1,hist_rgb1.size+1)),hist_rgb1,width=3)
plt.show()
...
#print(hist_rg)
plt.bar(np.array(range(1,hist_rg.size+1)),hist_rg,width=3)
plt.show()
...
num_bins_color = 5
plt.subplot(1,2,2)
hist_dxdy = histogram_module.dxdy_hist(img_gray.astype('double'), num_bins_color)
#print(hist_dxdy)
plt.bar(np.array(range(1,hist_dxdy.size+1)),hist_rg,width=3)
plt.show()
# add your code here
...
for imgidx1 in range(len(image_files1)):
  ...
  for imgidx2 in range(len(image_files2)):
    ...
    D = np.zeros( (len(hist_types), len(distance_types)) )
    #print(D.shape)
...
#query_images_vis = [query_images[i] for i in np.array([0,4,9,15,21,24,50,40,60])]
...
#print(list(range(len(query_images))))
#print(best_match)
num_correct = sum( best_match == list(range(len(query_images))) )
print('number of correct matches: %d (%f)\n'% (num_correct, 1.0 * num_correct / len(query_images)))

###
## experiments with diff hist type and num of bins
###
#results_file = open('results.csv','w')

def hist_analysis(model_images,query_images,hist_types,distance_types):
    print("Historgram,Distance,Bins,Num_Correct_matches,Percent_correct")
    with open('results.csv','w') as results_file:
        for ht in hist_types:
            for df in distance_types:
                for bins in range(10,31,5):
                    [best_match, D] = match_module.find_best_match(model_images,
                                                    query_images, df, ht, bins)

                    num_correct = sum(best_match == list(range(len(query_images))))
                    recog_percent = 1.0 * num_correct / len(query_images)
                    info = "{0},{1},{2},{3},{4}".format(ht, df, str(bins),
                                                    num_correct, recog_percent)
                    results_file.write(info + "\n")
                    print(info)

def plot_rpc_summary(model_images, query_images):

    eval_num_bins = list(range(10,31,5))

    i=1
    plt.figure()
    for bins in eval_num_bins:
        plt.subplot(1,5,i)
        rpc_module.compare_dist_rpc(model_images, query_images, ['chi2', 'intersect', 'l2'], 'rg', bins, ['r', 'g', 'b'])
        plt.title('RG histogram with #bins = ' + str(bins))
        i=i+1
    plt.show()
    ...
    for bins in eval_num_bins:
        plt.subplot(1,5,i)
        rpc_module.compare_dist_rpc(model_images, query_images, ['chi2', 'intersect', 'l2'], 'rgb', bins, ['r', 'g', 'b'])
        plt.title('RGB histogram with #bins = ' + str(bins))
        i = i + 1
    plt.show()
    ...
    for bins in eval_num_bins:
        plt.subplot(1,5,i)
        rpc_module.compare_dist_rpc(model_images, query_images, ['chi2', 'intersect', 'l2'], 'dxdy', bins, ['r', 'g', 'b'])
        plt.title('dx/dy histogram with #bins = ' + str(bins))
        i = i + 1
    plt.show()
...
# part of Q3 an Q4 - used to report summary that we included in PDF.
# uncomment to run
#hist_analysis(model_images,query_images,hist_types,distance_types)
#plot_rpc_summary(model_images, query_images)
