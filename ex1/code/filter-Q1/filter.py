...
plt.subplot(1,4,2)
# applying gaussian blur in Y-direction (vert)
plt.imshow(conv2(img, G.T, 'same'), cmap='gray')
plt.subplot(1,4,3)
# applying gaussian blur in X-direction (horz)
plt.imshow(conv2(img, G, 'same'), cmap='gray')
plt.subplot(1,4,4)
# applying gaussian blur in XY-direction
...
plt.figure()
plt.subplot(2,3,1)
plt.imshow(conv2(conv2(img, G, 'same'), G.T, 'same') , cmap='gray') # simple conv with gaussion in XY dir
plt.subplot(2,3,2)
plt.imshow(conv2(conv2(img, G, 'same'), D.T, 'same') , cmap='gray') # same - img drvtv in X-direction (horz)
plt.subplot(2,3,3)
plt.imshow(conv2(conv2(img, D.T, 'same'), G, 'same') , cmap='gray') # same - img drvtv in X-direction (horz)
plt.subplot(2,3,4)
plt.imshow(conv2(conv2(img, D, 'same'), D.T, 'same') , cmap='gray')
plt.subplot(2,3,5)
plt.imshow(conv2(conv2(img, D, 'same'), G.T, 'same') , cmap='gray') # same 2 - img drvtv in Y-direction (vert)
plt.subplot(2,3,6)
plt.imshow(conv2(conv2(img, G.T, 'same'), D, 'same') , cmap='gray') # same 2 - img drvtv in Y-direction (vert)
plt.show()
